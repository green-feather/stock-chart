...
module.exports = { 
  getOne: (selector) => {
    return client.query('select * from stockinfo where stockid=$1', [selector])
    .then((res) => {
      return res.rows[0];
    })
  ...
  updateOne: () => {
    const query = 'update stockinfo set changePercent=5.11 where stockId=$1';
    const selectors = ['PLUG000000001'];
    client.query(query, selectors)
    .then(res => {
      console.log('Updated!');
    })
    .then(() => client.end())
  ...
  deleteOne: () => {
    const query = 'delete from stockinfo where stockId=$1';
    const selectors = ['PLUG000000001'];
    client.query(query, selectors)
    .then(res => {
  ...
  postOne: () => {
    const query = "insert into stockinfo values ('000000001', 'Plug Power000000001', 'PLUG000000001', " + 
    "'102.81','5.11'," +
